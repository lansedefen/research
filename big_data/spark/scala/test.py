...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        ...
        data_input_path = "/Users/wangchengyun/work/spark-2.1.1-bin-hadoop2.7/examples/src/main/resources/people.txt"
    ...
    df = spark.read.load("/Users/wangchengyun/work/spark-2.1.1-bin-hadoop2.7/examples/src/main/resources/people.json", format="json")
    #df.select("name", "age").write.save("namesAndAges.parquet", format="parquet")
    #df = spark.sql("SELECT * FROM parquet.`/Users/wangchengyun/work/spark-2.1.1-bin-hadoop2.7/examples/src/main/resources/users.parquet`")
    #df.write.(path = "/tmp/xx8", compression = "none")

    #df2 = df.write.save("/tmp/xx11", format="json")

    sc = spark.sparkContext
    textFile = sc.textFile("/Users/wangchengyun/work/spark-2.1.1-bin-hadoop2.7/examples/src/main/resources/people.json")
    #rdd1 = sc.parallelize([1, 10, 2, 3, 4, 5])
    #xx = rdd1.sample(0.1)
    xx = sc.parallelize([1, 2, 1, 2, 2], 2)
    #xx1= xx.flatMap(lambda x: [(x, x), (x, x)])
    #xx1.saveAsSequenceFile("/tmp/xx23", "org.apache.hadoop.io.compress.DefaultCodec")

    ff= sc.sequenceFile("/tmp/xx23*", "org.apache.hadoop.io.Text", "org.apache.hadoop.io.LongWritable")

    #click_show_join_rdd = spark.read.text(data_input_path).rdd.map(lambda r: r[0])
    for k in ff.collect():
        print(111, k)

    #for line in df2.rdd.collect():
    #    print(line)

    df.write.saveAsTable("table2", mode = "overwrite")
    xx = spark.read.table("table2")
    for ele in xx.rdd.collect():
        print(ele)

    parquetFile = spark.read.parquet("/Users/wangchengyun/work/spark-2.1.1-bin-hadoop2.7/examples/src/main/resources/people.parquet")
    parquetFile.createOrReplaceTempView("parquetFile")
    teenagers = spark.sql("SELECT name FROM parquetFile WHERE age >= 13 AND age <= 19")
    teenagers.show()

    #spark.sql("CREATE TABLE IF NOT EXISTS src (key INT, value STRING)")
    #spark.sql("LOAD DATA LOCAL INPATH '/Users/wangchengyun/work/spark-2.1.1-bin-hadoop2.7/examples/src/main/resources/kv1.txt' INTO TABLE src")
