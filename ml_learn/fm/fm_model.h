...
int fm_model::loadModel(std::string model_file_path) {
	...
	if (model_file.is_open()){
		...
		if (k1) {
			if(!std::getline(model_file,line)){ std::cout << "line:" <<  line << std::endl;  return 0;} //"#unary interactions Wj"
			for (uint i = 0; i<num_attribute; i++){
				if(!std::getline(model_file,line)){ std::cout << "line2:" <<  line << std::endl; return 0;}
		...
		for (uint i = 0; i<num_attribute; i++){
			...
			for (int f = 0; f < num_factor; f++) {
				...
				std::cout << "f,i:" << f << "," << i <<"v(f,i):" << v(f,i) << std::endl;
			}
		}
		model_file.close();
	}
